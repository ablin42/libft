...
int	main()
{
	...
	printf("ME(Entropy, Rapture, 4)		: %s	||	", ft_strncpy(ent, rap, 4));
	printf("LIB	: %s\n", strncpy(ent, rap, 4));
	
	printf("ME(Destruction, Void, 8)	: %s		||	", ft_strncpy(des, voi, 8));
	printf("LIB	: %s\n", strncpy(des, voi, 8));
	//CRASH TEST OVERFLOW
	printf("ME(Kill, Exterminate, 13)	: %s	||	", ft_strncpy(kill, ext, 13));
	printf("LIB	: %s\n", strncpy(kill, ext, 13));
	return (0);
}